...
<title>공지사항 게시판</title>
		...
		ResultSet rset = stmt.executeQuery("select * from notice_re order by rootid desc, recnt asc, relevel asc limit " + frontNum + "," + cnt + ";");
		%>
		</head>
	<body>
	<table cellspacing=1 width=650 border=1>
		...
		%>
		<td width=50>
			<p><a href="ReNotice_view.jsp?key=<%=rset.getInt(1)%>"><%=title%></a></p>
		</td>
		<%
		...
		out.println("<td width = 50><p  align='center'>" + rset.getInt(1) + "</p></td>");
		%>
		<td width=50><p><a href="ReNotice_view.jsp?key=<%=rset.getInt(1)%>">
				<%
